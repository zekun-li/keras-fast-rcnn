...
def bbox_regressor( nb_classes = nb_classes ) :
    ...
    '''
    bbox = Sequential()
    bbox.add(Dense(4096, input_shape = (512 * target_h * target_w, ), activation = 'relu'))
    bbox.add(Dropout(0.5))
    bbox.add(Dense(4096, activation = 'relu'))
    bbox.add(Dropout(0.5))
    bbox.add(Dense(4*nb_classes, activation = 'linear'))
    
    for bl, vl in zip( bbox.layers, vgg.layers[38:]):
        try:
            bl.set_weights( vl. get_weights())
        except Exception, e:
            print "WARNING: can not set weights for", bl, e, "in bbox_regressor, skipped"
