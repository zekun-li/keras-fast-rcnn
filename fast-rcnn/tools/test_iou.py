...
if __name__ == '__main__':
    ...
    '''
    pred_data_list = roidb
    pred = datagen(pred_data_list, nb_epoch = 1)
    print 'predicting...'
    tic = time.clock()
    out = fastrcnn.fast.predict_generator(pred, len(pred_data_list))
    toc = time.clock()
    print 'done prediction, used %d secs' %(toc-tic)
    '''
    ...
    for i in range(0,len(roidb)):
        ...
        bbox_delta_target = roidb[i]['bbox_targets'][:,1:]
        bbox_label_target = roidb[i]['bbox_targets'][:,0]
        bbox_label_target = bbox_label_target.astype(np.int32)
        orig_boxes = roidb[i]['boxes']
        bbox_delta_out = bbox_delta_out[0]
        pred_boxes = _bbox_pred(orig_boxes, bbox_delta_out)
        # CLIP PRED_BOXES
        im_shape = X[0].shape
        pred_boxes = _clip_boxes(pred_boxes, im_shape)

        target_boxes = _bbox_pred(orig_boxes, bbox_delta_target)
        for ind in range(0, len(bbox_delta_out)):
            if (bbox_label_target[ind] != 0):
                iou = compute_iou(pred_boxes[ind], target_boxes[ind]) 
                cnt_20[bbox_label_target[ind]-1] = cnt_20[bbox_label_target[ind]-1]+1
                sum_20[bbox_label_target[ind]-1] = sum_20[bbox_label_target[ind]-1]+ iou



    avg_20 = sum_20/cnt_20
    print avg_20
    print 'done'
    ...
    for i in range(0,len(roidb)):
        ...
        #    pickle.dump({'pred_boxes':pred_boxes, 'pred_labels':pred_labels,'pred_conf':pred_conf},f)
        im_shape = X[0].shape
        pred_boxes = _clip_boxes(pred_boxes, im_shape)
